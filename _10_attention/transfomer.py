...
class transfomer_decoder(nn.Module):
    ...
    def forward(self,
        ...
        for blk in self.sequential:
            ...
            i += 1
        output = self.output_layer(q)
        return output,self.attention_weights,self.cross_attention_weights


def create_masks(src_seq, tgt_seq, pad_token_id=0, device='cpu'):
    """
    ...
    src_mask = (src_seq == pad_token_id).unsqueeze(1).unsqueeze(2)

    # 2. 创建目标序列填充掩码（用于解码器自注意力）
    # shape: (batch_size, 1, 1, tgt_len)
    tgt_padding_mask = (tgt_seq == pad_token_id).unsqueeze(1).unsqueeze(2)

    # 3. 创建前瞻掩码（上三角矩阵，不包括对角线）
    ...
    look_ahead_mask = look_ahead_mask.expand(batch_size, -1, -1, -1)
    ...
    # 6. 合并目标序列掩码（填充+前瞻）
    # 注意：两者都是True表示要屏蔽，所以用逻辑或
    tgt_mask = tgt_padding_mask_expanded | look_ahead_mask

    # 7. 编码器输出掩码（用于解码器交叉注意力）
    # 与源序列填充掩码相同，形状: (batch_size, 1, 1, src_len)
    memory_mask = src_mask.clone()

    # 8. 计算有效长度（可选，如果需要使用valid_len参数）
    src_valid_len = (src_seq != pad_token_id).sum(dim=1)  # (batch_size,)
    tgt_valid_len = (tgt_seq != pad_token_id).sum(dim=1)  # (batch_size,)

    return {
        'src_mask': src_mask,  # (batch_size, 1, 1, src_len)
        'tgt_mask': tgt_mask,  # (batch_size, 1, tgt_len, tgt_len)
        'memory_mask': memory_mask,  # (batch_size, 1, 1, src_len)
        'src_valid_len': src_valid_len,  # (batch_size,)
        'tgt_valid_len': tgt_valid_len  # (batch_size,)
    }
...
def test_encoder_decoder():
    ...
    x1 = torch.randint(0, vocab_size, (batch_size, seq_len)) #源序列
    x2 = torch.randint(0, vocab_size, (batch_size, seq_len)) #目标序列
    #填充
    ...
    show_multiple_heatmaps(
        ...
        filename="encoder_attention_weights",
        hide_ticks=True
    )
    show_multiple_heatmaps(
        ...
        filename = "decoder_attention_weights",
        hide_ticks = True
    ...
    show_multiple_heatmaps(
        ...
        filename = "cross_attention_weights",
        hide_ticks = True
    )
